...
useEffect(() => {
    const saveProgress = async () => {
        try {
            ...
            const completionPercentage = Math.min((currentLength / totalPossibleLength) * 100, 100);
    ...
    const loadProgress = async () => {
        try {
            ...
            if (response.data.success && response.data.progress) {
                ...
                const savedAnswers = typeof answersGiven === 'string' ? JSON.parse(answersGiven) : answersGiven;
                // Check if sessionData is already an object
                const savedSession = sessionData ? (typeof sessionData === 'string' ? JSON.parse(sessionData) : sessionData) : null;
